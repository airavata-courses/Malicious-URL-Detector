"""
Refactored Intensive ML Model Script with Timestamped Outputs and HPC Enhancements
...
This script includes:
    ...
    - HPC enhancements to leverage a multi-core/multi-node HPC environment.
...
# HPC Enhancement: Set non-interactive backend for headless HPC environments.
matplotlib.use('Agg')
...
# HPC Enhancement: Use solver 'saga' and n_jobs=-1 to leverage all cores.
from sklearn.linear_model import LogisticRegression  # type: ignore 
from sklearn.svm import LinearSVC  # type: ignore 
# HPC Enhancement: Use n_jobs=-1 for RandomForest to use all available cores.
from sklearn.ensemble import RandomForestClassifier  # type: ignore 
from sklearn.tree import plot_tree  # type: ignore 
from joblib import Parallel, delayed  # type: ignore 

import warnings
import concurrent.futures  # HPC Enhancement: Import concurrent.futures for parallel model training.
...
def run_logistic_regression_model(X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names, output_folder):
    ...
    # HPC Enhancement: Use solver 'saga' and n_jobs=-1 to use all available cores.
    logreg = LogisticRegression(solver='saga', n_jobs=-1, max_iter=1000)
...
def run_random_forest_model(X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names, output_folder):
    ...
    # HPC Enhancement: Set n_jobs=-1 to utilize all available cores.
...
def main():
    ...
    Steps:
        ...
        4. Train and evaluate multiple machine learning models concurrently.
        5. Report the overall execution time and CO2 emissions.
    
    HPC Enhancement (Suggestion):
        - If GPU acceleration libraries (e.g., cuML from RAPIDS) are available,
          consider replacing scikit-learn models with their GPU-accelerated counterparts.
    """
    ...
    # HPC Enhancement: Run model training concurrently using ProcessPoolExecutor.
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        futures.append(executor.submit(run_lda_model, X_train, X_test, y_train, y_test, le, X.columns, run_folder))
        futures.append(executor.submit(run_logistic_regression_model, X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns, run_folder))
        futures.append(executor.submit(run_svm_model, X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns, run_folder))
        futures.append(executor.submit(run_random_forest_model, X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns, run_folder))
        # Wait for all model training tasks to complete.
